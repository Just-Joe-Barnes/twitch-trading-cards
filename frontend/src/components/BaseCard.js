...
const BaseCard = ({
    ...
    useEffect(() => {
        ...
        if (card) {
            // Ensure initial transform includes perspective for 3D effect
    ...
    useEffect(() => {
        if (descriptionRef.current) {
            // Only apply font size adjustment for full-size cards
            if (!miniCard) {
                ...
                // For mini cards, ensure a base font size if not handled by CSS
                descriptionRef.current.style.fontSize = '0.7rem'; // Or whatever fits best for mini
            }
        }
    ...
    useEffect(() => {
        if (nameRef.current) {
            ...
            if (!miniCard) {
                ...
                // For mini cards, ensure a base font size if not handled by CSS
                nameRef.current.style.fontSize = '0.75rem'; // Or whatever fits best for mini
            }
        }
    }, [name, miniCard]);
    ...
    const TimeStatusBadge = ({card, now}) => {
        ...
        if (timeLeft <= 0) {
            return (
                <div className={`${badgeClass} expired pulse`}>
                    Expired
                </div>
            );
        }
            ...
            const minutes = Math.floor((timeUntilStarts % (1000 * 60 * 60)) / (1000 * 60));
            const seconds = Math.floor((timeUntilStarts % (1000 * 60)) / 1000);
            return (
                <div className={`${badgeClass} upcoming`}>
            ...
            return (
                <div className={`${badgeClass} pulse`}>
    ...
    const handlePointerMove = (e) => {
        const card = cardRef.current;
        card.style.transitionDuration = '.1';
        ...
        if (rarity.toLowerCase() === 'unique' && invertRef.current) {
            ...
            const spread = 27; // Keep consistent
        ...
        // Adjust tilt sensitivity for mini card
        ...
        if (rarity.toLowerCase() === 'legendary') {
            const lx = ((x / rect.width) * (miniCard ? 5 : 10) - (miniCard ? 2.5 : 5)).toFixed(2) + '%'; // Adjusted for mini card
            const ly = ((y / rect.height) * (miniCard ? 5 : 10) - (miniCard ? 2.5 : 5)).toFixed(2) + '%'; // Adjusted for mini card
        ...
        if (modifierData?.name === 'Glitch') {
            const glitchSensitivity = miniCard ? 30 : 15; // Adjusted for mini card
    ...
    // Unified pointer leave/end/cancel handler
    ...
    return (
        <>
            ...
            >
                <div className={`${miniCard ? 'mini-' : ''}card-content-3d-wrapper ${slabbed ? 'slabbed' : ''}`}>
                    {["basic", "common", "standard", "uncommon"].includes(rarity.toLowerCase()) && (
                        ...
                        {rarity.toLowerCase() === 'divine' ? (
                            <>
                                <div className={cardHeaderClass}>
                                    <div
                                        className={`${cardNameClass} ${isGlitch ? 'glitch-text' : ''}`}
                                        ref={nameRef}
                                        data-text={name}
                                    >
                                        {name}
                            ...
                            <>
                                ...
                                {(!miniCard && description) && (
                                    <div
                                        className={`${cardDescriptionClass} ${isGlitch ? 'glitch-text' : ''}`}
                                        ref={descriptionRef}
                                ...
                                <div
                                    className={`${cardMintClass} ${isGlitch ? 'glitch-text' : ''}`}
                                    data-text={`${mintNumber} / ${rarities.find(r => r.name.toLowerCase() === rarity.toLowerCase())?.totalCopies ?? '???'}`}
                                >
                                  <span className="mint-number">
                                    {mintNumber} /{' '}
                ...
                {slabbed && (
                    <div className={`slab-overlay ${rarity.toLowerCase()}`}
                ...
                {limited && (
                    <div className={`limited-overlay ${miniCard ? 'mini' : ''}`}/>
                )}
            </div>
        </>
    );
