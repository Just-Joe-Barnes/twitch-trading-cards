...
const stripCardNameModifiers = (cardName) => {
    ...
    const modifiers = ["Glitched ", "Negative ", "Prismatic "];
    for (const modifier of modifiers) {
        if (cleanedName.startsWith(modifier)) {
            cleanedName = cleanedName.substring(modifier.length);
            break;
        }
    }
    return cleanedName;
...
router.get('/audit-cards', protect, adminOnly, async (req, res) => {
    ...
    if (mongoose.connection.readyState !== 1) {
        ...
        auditResults.message = "Database not connected. Please ensure MongoDB is running and connected in your main application.";
    ...
    try {
        ...
        for (const user of users) {
            ...
            for (const userCard of user.cards) {
                ...
                if (!userCard.name || !userCard.rarity || typeof userCard.mintNumber !== 'number') {
                    auditResults.details.malformedUserCards.push({
                        userId: user._id,
                        username: user.username,
                        cardIndex: cardIndex,
                        userCardData: userCard,
                        issue: "Malformed or incomplete card data (missing name, rarity, or mintNumber)"
                    });
                    continue;
                }
                ...
                if (!parentCardId) {
                    auditResults.details.missingParentCardDefinitions.push({
                        userId: user._id,
                        username: user.username,
                        cardIndex: cardIndex,
                        userCardData: userCard,
                        cleanedCardName: cleanedUserCardName,
                        issue: "No matching parent card definition found (after stripping modifiers)"
                    });
                    continue;
                }
                ...
                if (!rarityDefinition) {
                    auditResults.details.missingRarityDefinitions.push({
                        userId: user._id,
                        username: user.username,
                        cardIndex: cardIndex,
                        userCardData: userCard,
                        ...
                        issue: "No matching rarity definition found within parent card (after stripping modifiers)"
                    });
                    continue;
                }
                ...
                if (availableMintsInCardDef.includes(userCardMintNumber)) {
                    auditResults.details.duplicateMintNumbersInCardDef.push({
                        userId: user._id,
                        username: user.username,
                        cardIndex: cardIndex,
                    ...
                    if (potentialNewMints.length > 0) {
                        ...
                        auditResults.details.userCardsToReroll.push({
                            userId: user._id,
                            username: user.username,
                            cardIndex: cardIndex,
                            cardName: userCardName,
                            cleanedCardName: cleanedUserCardName,
                            rarity: userCardRarityLower,
                            oldMintNumber: userCardMintNumber,
                            suggestedNewMintNumber: suggestedNewMintNumber,
                            parentCardId: parentCardId,
                            issue: "Mint number marked as available in Card definition"
                        });
                    } else {
                        auditResults.details.noMintNumbersLeftForReroll.push({
                            userId: user._id,
                            username: user.username,
                            cardIndex: cardIndex,
                            cardName: userCardName,
                            ...
                            mintNumber: userCardMintNumber,
                            parentCardId: parentCardId,
                            issue: "Inconsistency found but no other mints available in Card definition for reroll"
                        });
                    }
                }
            }
        ...
        for (const [uniqueCardInstanceKey, owners] of globalAssignedMintsTracker.entries()) {
            if (owners.length > 1) {
                owners.sort((a, b) => {
                    const dateA = a.acquiredAt ? new Date(a.acquiredAt).getTime() : Infinity;
                    const dateB = b.acquiredAt ? new Date(b.acquiredAt).getTime() : Infinity;
                    return dateA - dateB;
                });
                ...
                auditResults.details.trueDuplicateCardInstancesAcrossUsers.push({
                    cardInstance: uniqueCardInstanceKey,
                    owners: owners.map(o => ({
                        userId: o.userId,
                        username: o.username,
                        cardIndex: o.cardIndex,
                        acquiredAt: o.acquiredAt,
                        isFirstOwner: o.isFirstOwner || false
                    }))
                });
            }
        }
